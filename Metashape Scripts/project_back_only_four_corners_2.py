...
def process_camera(chunk,camera, img, tile,tile_lable,output_path):
    ...
    if(len(corner_in_image)==4):
        ...
        min_x = min(corner_in_image, key=lambda point: point[0])[0]
        min_y = min(corner_in_image, key=lambda point: point[1])[1]
        max_x = max(corner_in_image, key=lambda point: point[0])[0]
        max_y = max(corner_in_image, key=lambda point: point[1])[1]

        if corner_in_image[0][0] > corner_in_image[3][0] and corner_in_image[0][1] > corner_in_image[3][1]:
            rotation_angle = -90
        else:
            rotation_angle = 90
        width = max_x - min_x
        ...
        new_image[:,:]= img[corner_in_image[2][1]:corner_in_image[0][1],corner_in_image[0][0]:corner_in_image[1][0]] 

        height, width = new_image.shape[:2]
        center = (width // 2, height // 2)

        rotation_matrix = cv2.getRotationMatrix2D(center, rotation_angle, 1.0)
        rotated_image = cv2.warpAffine(new_image, rotation_matrix, (width, height)) 

        cv2.imwrite(output_path + camera.label+'_'+ tile_lable + ".png", rotated_image)
